...
def AGM(
    ...
    for i in range(len(num_channels_down)):
        ...
        if num_channels_skip[i] != 0:
            model_tmp.add(Concat(1, AGM, deeper))
        else:
        ...
        if num_channels_skip[i] != 0:
            AGM.add(conv(input_depth, num_channels_skip[i], filter_skip_size, bias=need_bias, pad=pad))
            AGM.add(bn(num_channels_skip[i]))
            AGM.add(act(act_fun))
        ...
        deeper.add(MambaHSI(in_channels=num_channels_down[i], hidden_dim=128))
        ...
        model_tmp.add(MambaHSI(in_channels=num_channels_up[i], hidden_dim=128))
